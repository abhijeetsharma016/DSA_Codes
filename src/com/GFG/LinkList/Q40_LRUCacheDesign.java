...
class Node40 {
    ...
    public Node40(int key, int value)
    {
        this.key = key;
        this.value = value;
    }
...
class LRUCache {
    private HashMap<Integer, Node40> map;
    private int capacity, count;
    private Node40 head, tail;
    ...
    public void deleteNode(Node40 node)
    {
        node.pre.next = node.next;
        node.next.pre = node.pre;
    }

    public void addToHead(Node40 node)
    {
        node.next = head.next;
        node.next.pre = node;
        node.pre = head;
    ...
    public void set(int key, int value)
    {
        ...
        if (map.get(key) != null) {
            ...
            Node40 node = new Node40(key, value);
            map.put(key, node);
            if (count < capacity) {
                count++;
                addToHead(node);
...
public class Q40_LRUCacheDesign {
